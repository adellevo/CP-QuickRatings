...
//   const largetTargetsExpanded = document.querySelectorAll<HTMLElement>(
//     ".cx-MuiCollapse-container .cx-MuiGrid-root:nth-child(2) > div > div:nth-child(3)"
//   );
...
//     if (!professor || section.classList.contains(POPUP_PARENT_CONTAINER_CLASS)) {
//       // Can't do anything since we did not find a professor or we already added the popup
//       return;
//     }
//     section.classList.add(POPUP_PARENT_CONTAINER_CLASS);

//     // FOR COLORS. IGNORE FOR NOW
//     // On the smaller version the flex stretches when there is no reason for it
//     // if (smallerTargets.length) {
...
//     const prof = await findProfessor(professorName);
...
// const findProfessor = async (name: string) => {
